...
kubectl create -f yamls/openldap.yaml 2> /dev/null || true
echoGreenBold 'openldap service created...'
# Create the phpldapadmin service  
kubectl create -f yamls/phpldapadmin.yaml 2> /dev/null || true
...
kubectl create -f yamls/email.yaml 2> /dev/null || true
echoGreenBold 'email service created...'
#Building docker image

# Create the persistent volume and persistent volume claim for database
kubectl create -f yamls/mysql-pv.yaml  2> /dev/null || true
echoGreenBold 'Persistent Volume created...'
# Create mysql deployment
kubectl create -f yamls/mysql-deployment.yaml  2> /dev/null || true
...
kubectl create -f yamls/prometheus-alert/clusterRole.yaml 2> /dev/null || true
echoGreenBold 'Role creation and Role binding...'

# Create the config map to keep configuration data of prometheus
kubectl create -f yamls/prometheus-alert/config-map.yaml -n copper 2> /dev/null || true
echoGreenBold 'Prometheus configuration created...'

# Deploy prometheus pods 
kubectl create  -f yamls/prometheus-alert/prometheus-deployment.yaml --namespace=copper 2> /dev/null || true

# Create the service to access prometheus 
kubectl create -f yamls/prometheus-alert/prometheus-service.yaml --namespace=copper 2> /dev/null || true
echoGreenBold 'Prometheus service created...'
# Alert manager implementation
# Creating the configuration 
kubectl create -f yamls/prometheus-alert/AlertManagerConfigmap.yaml 2> /dev/null || true
#
kubectl create -f yamls/prometheus-alert/AlertTemplateConfigMap.yaml 2> /dev/null || true
echoGreenBold 'Alert Manager congiguration created..'
#
kubectl create -f yamls/prometheus-alert/Deployment.yaml 2> /dev/null || true
#
kubectl create -f yamls/prometheus-alert/Service.yaml 2> /dev/null || true
...
kubectl create -f yamls/groupoffice.yaml 2> /dev/null || true
